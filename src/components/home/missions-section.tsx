...
const SECTOR_ICON: Record<string, any> = {
  fullstack: Radar,
  mobile: Cast,
  desktop: Microchip,
};
                  ...
                  <div className="space-y-2">
                    {blocModules.map((mod) => {
                      ...
                      if (mod.available) {
                        return (
                          <Link
                            key={mod.id}
                            href={mod.href}
                            className="flex items-center gap-3 rounded-lg border border-white/5 bg-white/2 px-3 py-2.5 transition-all hover:border-lyoko-blue/30 hover:bg-lyoko-blue/5"
                          >
                      ...
                      return (
                        <div
                          key={mod.id}
                          className="flex items-center gap-3 rounded-lg border border-white/5 bg-white/2 px-3 py-2.5 opacity-50"
                        >
                          <ModIcon size={18} style={{ color: mod.color }} />
                          <span className="flex-1 text-sm font-medium text-white/80">
                            {mod.name}
