...
export function ExerciseClient({ moduleId, moduleName, moduleColor, moduleLevels }: Props) {
  ...
  // Queue of newly completed exercise IDs to process in useEffect
  const newlyCompletedRef = useRef<string[]>([]);
  const comboCountRef = useRef(0); // mirror of comboCount for useEffect reads
  const migrationDoneRef = useRef(false);

  // Keep comboCountRef in sync
  comboCountRef.current = comboCount;

  // Helper: fire an XP toast
  ...
  // Load exercises from registry
  useEffect(() => {
    const data = getExercises(moduleId);
    if (data.length > 0) {
      setExercises(data);
    }
    // Pre-populate shownMilestones so we don't re-fire on page load
  ...
  }, [exercises.length]); // only on initial load
  ...
  // ── markComplete: PURE — only touches completed Set ──
  // All side effects (addXp, combo, toasts) run in the useEffect below.
  const markComplete = useCallback((id: string) => {
    setCompleted((prev) => {
      if (prev.has(id)) return prev;
      // Queue the ID for side-effect processing
  ...
  // ── Side effect processor — runs AFTER render, never during ──
  useEffect(() => {
    ...
    // Milestone check (after all IDs processed)
    if (exercises.length > 0) {
      ...
      for (const m of MILESTONES) {
        if (pct >= m && !shownMilestonesRef.current.has(m)) {
          ...
          setTimeout(() => setMilestone({ pct: m }), 800);
          if (m === 100) { addXp(250); play("sectionComplete"); }
          else play("badge");
          break;
        }
      }
  ...
  const goNext = useCallback(() => {
    if (currentIdx < exercises.length - 1) {
      ...
      setTimeout(() => { setCurrentIdx(currentIdx + 1); setHelpLevel(0); setTimeout(() => setTransitioning(false), 50); }, 150);
    }
  ...
  // Skip to next uncompleted exercise
  const skipToNextUncompleted = useCallback(() => {
    for (let i = currentIdx + 1; i < exercises.length; i++) {
      if (!completed.has(exercises[i].id)) {
        play("transition");
        setTransitioning(true);
        setTimeout(() => { setCurrentIdx(i); setHelpLevel(0); setTimeout(() => setTransitioning(false), 50); }, 150);
        return;
      }
    }
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      // Ctrl+Enter — run code (bubble up to CodeView) or validate
      if ((e.ctrlKey || e.metaKey) && e.key === "Enter") {
        // Let CodeView handle it via its own handler
        return;
      }
  ...
  return (
    <div className={`flex relative transition-all duration-300 ${zenMode ? "h-[100dvh] fixed inset-0 z-[100]" : "h-[calc(100dvh-4rem)]"}`}>
      <FranzCodePlayer />
      ...
      {/* ═══ SIDEBAR ═══ */}
      <aside
        className={`flex-shrink-0 overflow-y-auto border-r border-white/5 bg-dark-surface transition-all duration-300 ${zenMode ? "w-0 overflow-hidden" : sidebarOpen ? "w-72" : "w-0 overflow-hidden"
          }`}
      >
        <div className="p-4">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        {/* Toolbar */}
        <div className={`flex items-center gap-2 border-b border-white/5 bg-dark-bg px-4 py-2 transition-all ${zenMode ? "opacity-0 h-0 py-0 overflow-hidden" : ""}`}>
          {/* Breadcrumbs */}
          <div className="flex items-center gap-1.5 mr-2">
            ...
            {exercise && <><ChevronRight size={12} className="text-white/15" /><span className="text-xs font-medium text-white/60 truncate max-w-[150px]">#{currentIdx + 1}</span></>}
          </div>
          ...
          {/* ⚡ Combo badge */}
          {comboCount >= 3 && (
            <div className={`flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[0.65rem] font-bold transition-all ${comboCount >= 6
              ? "border border-xana-red/40 bg-xana-red/15 text-xana-red shadow-[0_0_10px_rgba(255,34,68,0.3)]"
              : "border border-carthage-gold/40 bg-carthage-gold/15 text-carthage-gold"
              }`}>
              🔥 ×{comboCount >= 6 ? 3 : 2} COMBO
            </div>
          )}

          {/* Skip to next uncompleted */}
          <button
            onClick={skipToNextUncompleted}
            title="Prochain non-complété (S)"
            className="rounded p-1 text-white/30 hover:bg-lyoko-blue/10 hover:text-lyoko-blue transition-colors"
          >
            <SkipForward size={16} />
          </button>
          ...
          <button
            ...
            title="Mode Focus (F)"
            className="rounded p-1 text-white/30 hover:bg-lyoko-purple/10 hover:text-lyoko-purple transition-colors"
          >
        ...
        {/* Zen mode mini-nav */}
        {zenMode && (
          <div className="absolute top-2 right-2 z-10 flex items-center gap-1 rounded-lg border border-white/10 bg-dark-surface/90 px-2 py-1 backdrop-blur-sm">
            ...
            <span className="text-[0.65rem] text-white/40">{currentIdx + 1}/{exercises.length}</span>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          >
            {!exercise ? (
              ...
              <QuizView ex={exercise} onComplete={() => markComplete(exercise.id)} goNext={goNext} isCompleted={completed.has(exercise.id)} onWrong={() => setComboCount(0)} />
            ) : exercise.type === "puzzle" ? (
              <PuzzleView ex={exercise} onComplete={() => markComplete(exercise.id)} goNext={goNext} isCompleted={completed.has(exercise.id)} onWrong={() => setComboCount(0)} />
            ) : (
              <CodeView ex={exercise} onComplete={() => markComplete(exercise.id)} goNext={goNext} isCompleted={completed.has(exercise.id)} helpLevel={helpLevel} setHelpLevel={setHelpLevel} moduleId={moduleId} onWrong={() => setComboCount(0)} />
            )}
          </div>
        </div>
      </div>
...
function SidebarItem({ ex, isCurrent, isCompleted, onClick, color }: {
  ...
  return (
    <button
      onClick={onClick}
      className={`flex w-full items-center gap-2 rounded-lg px-2.5 py-1.5 text-left text-xs transition-all ${isCurrent ? "border border-white/10 bg-white/8 text-white" : "text-white/60 hover:bg-white/5 hover:text-white/80"
        }`}
    >
      {isCompleted ? (
    ...
    <div className="mx-auto max-w-3xl">
      ...
      <div className="text-center">
        <button
          ...
          className={`inline-flex items-center gap-2 rounded-lg px-8 py-3 text-sm font-semibold transition-all ${done || isCompleted
...
function QuizView({ ex, onComplete, goNext, isCompleted, onWrong }: { ex: Exercise; onComplete: () => void; goNext: () => void; isCompleted: boolean; onWrong?: () => void }) {
  ...
  useEffect(() => {
    setSelected(null); setValidated(false); setIsCorrect(false);
    setTimeLeft(QUIZ_TIME); setTimerActive(true); startTimeRef.current = Date.now();
  }, [ex]);
  ...
  useEffect(() => {
    ...
    if (timeLeft <= 0) {
      ...
      onWrong?.();
      play("error");
      setTimeout(() => { setValidated(false); setSelected(null); setIsCorrect(false); setTimeLeft(QUIZ_TIME); setTimerActive(true); startTimeRef.current = Date.now(); }, 2000);
      return;
    }
    ...
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {/* Timer */}
          {!isCompleted && (
            <div className={`flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-bold transition-colors ${timeLeft <= 10
              ? "border-xana-red/40 bg-xana-red/10 text-xana-red animate-pulse"
              : "border-lyoko-blue/30 bg-lyoko-blue/10 text-lyoko-blue"
              }`}>
              ⏱ {timeLeft}s
            </div>
          )}
      ...
      {!isCompleted && (
        <div className="mb-4 h-1 overflow-hidden rounded-full bg-white/5">
          <div
            className={`h-full rounded-full transition-all duration-1000 ${timeLeft <= 10 ? "bg-xana-red" : "bg-lyoko-blue"}`}
      ...
      <div className="mb-5 space-y-2">
        {ex.options?.map((opt, i) => {
          ...
          return (
            ...
            >
              <span className={`flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full text-xs font-bold ${!validated && thisSelected ? "bg-carthage-gold/20 text-carthage-gold" : "bg-white/[0.06] text-white/50"
                }`}>
...
function PuzzleView({ ex, onComplete, goNext, isCompleted, onWrong }: { ex: Exercise; onComplete: () => void; goNext: () => void; isCompleted: boolean; onWrong?: () => void }) {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className={`mb-4 flex min-h-[80px] flex-wrap gap-2 rounded-xl border-2 border-dashed p-3 transition-colors ${result === "success" ? "border-lyoko-green/30 bg-lyoko-green/[0.03]" :
        ...
        {target.map((piece, idx) => (
          <button
            ...
            className={`${pieceBaseClass} ${result === "success" ? "border border-lyoko-green/40 bg-lyoko-green/[0.12] text-lyoko-green" :
...
function MobileEmulatorPanel({ code }: { code: string }) {
  ...
  const phoneH = isPortrait ? 420 : 280;
...
function CodeView({ ex, onComplete, goNext, isCompleted, helpLevel, setHelpLevel, moduleId, onWrong }: {
  ex: Exercise; onComplete: () => void; goNext: () => void; isCompleted: boolean; helpLevel: number; setHelpLevel: (n: number) => void; moduleId: string; onWrong?: () => void;
}) {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className={`mb-3 flex flex-1 gap-3 ${!showPreview || !hasPanel ? "flex-col" : ""}`} style={{ minHeight: 450 }}>
        {/* Editor Panel */}
      ...
      {/* Console output */}
      {output && (
        <div className={`mb-3 rounded-xl border p-3 font-mono text-sm ${status === "success" ? "border-lyoko-green/30 bg-lyoko-green/5 text-lyoko-green" : "border-xana-red/30 bg-xana-red/5 text-xana-red"
          }`}>
          {output}
        </div>
      )}
