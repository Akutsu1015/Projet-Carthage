...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === "login") {
      ...
      console.log("[Login] Lookup by username:", dbUser ? "found" : "not found");
      if (!dbUser) {
        dbUser = getUserByEmail(emailOrUsername);
        console.log("[Login] Lookup by email:", dbUser ? "found" : "not found");
      }
      ...
      if (!dbUser.password_hash) {
        return NextResponse.json({ success: false, error: "Ce compte utilise la connexion Google. Veuillez utiliser le bouton 'Continuer avec Google'." }, { status: 401 });
      }

      const passwordValid = verifyPassword(password, dbUser.password_hash);
      console.log("[Login] Password verification:", passwordValid ? "success" : "failed");
      if (!passwordValid) {
      ...
      const res = NextResponse.json({ success: true, user: userData, token });
...
export async function GET(req: NextRequest) {
  try {
    const token = getTokenFromRequest(req);
    if (!token) {
      // Pas de log ici pour ne pas flood, c'est le cas normal hors connexion
      return NextResponse.json({ success: false, user: null, error: "no_token" });
    }
    console.log(`[Session] Checking token: ${token.substring(0, 8)}...`);

    const dbUser = getSessionUser(token);
    if (!dbUser) {
      console.warn(`[Session] Token invalid or expired: ${token.substring(0, 8)}...`);
      const res = NextResponse.json({ success: false, user: null, error: "token_invalid" });
      res.cookies.delete("carthage_session");
      return res;
    }

    console.log(`[Session] User authenticated: ${dbUser.email} (ID: ${dbUser.id})`);

    // Block unverified users (except OAuth users who are auto-verified)
    if (!dbUser.email_verified) {
      console.warn(`[Session] User unverified: ${dbUser.email}. Deleting session...`);
      const res = NextResponse.json({ success: false, user: null, requiresVerification: true, error: "email_unverified" });
      res.cookies.delete("carthage_session");
      return res;
    }
