...
import {
  getUserByEmail, createOAuthUser, createSession, upgradeUserFromOAuth,
} from "@/lib/db";
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log(`[OAuth] Exchanging code for user: ${code.substring(0, 10)}...`);
    ...
    console.log(`[OAuth] Success! Creating session for user ${dbUser.id} (${dbUser.email})`);
    ...
    console.log(`[OAuth] Cookie set: carthage_session=${token.substring(0, 5)}...`);
