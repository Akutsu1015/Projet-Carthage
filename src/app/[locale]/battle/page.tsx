...
const DIFF_META: Record<string, { label: string; color: string; icon: any }> = {
...
const MODE_META: Record<string, { label: string; desc: string; color: string; icon: any }> = {
    ...
    <div className="min-h-[calc(100vh-4rem)]">
          ...
          <div className="flex flex-wrap items-center gap-6">
            <div className="flex-1">
              <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-xana-red/30 bg-xana-red/10 px-4 py-1.5 text-sm font-semibold text-xana-red">
                <Swords size={16} /> {t("battle_ext.code_battle")}
              </div>
              <h1 className="mb-2 font-display text-2xl font-extrabold md:text-3xl">
                <span className="bg-gradient-to-r from-xana-red to-carthage-gold bg-clip-text text-transparent">
                  {t("battle_ext.hero_title")}
                </span>
              </h1>
              <p className="max-w-lg text-sm text-white/50">
                {t("battle_ext.hero_subtitle").replace('%{count}', challengeCount ? challengeCount.total.toString() : "500")}
              </p>
            </div>
            ...
            {user && stats && (
              <div className="flex gap-3">
                ...
                <div className="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 text-center">
                  ...
                  <p className="text-[0.65rem] text-white/40">{t("battle_ext.wins")}</p>
                </div>
                <div className={`rounded-xl border px-3 py-3 text-center ${socket.connected ? "border-white/10 bg-white/[0.03]" : "border-xana-red/30 bg-xana-red/10"}`}>
                  ...
                  <p className="text-[0.65rem] text-white/40">{socket.connected ? t("battle_ext.online") : t("battle_ext.offline")}</p>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="mx-auto max-w-5xl px-4 py-6 lg:px-8">
        ...
        <div className="mb-6 flex flex-wrap items-center gap-2 border-b border-white/5 pb-3">
          {(Object.entries(MODE_META) as [string, typeof MODE_META[string]][]).map(([key, { label, icon: Icon, color }]) => (
            <button
              ...
              className={`flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-all ${tab === key ? "text-white" : "text-white/40 hover:text-white/60"
                }`}
              style={tab === key ? { background: `${color}15`, color } : {}}
            >
              <Icon size={15} /> {t(`battle_ext.modes.${key}`)}
            </button>
          ))}
          ...
          >
            <TrendingUp size={15} /> {t("battle_ext.tab_leaderboard")}
          </button>
          <button
          ...
          >
            <BarChart3 size={15} /> {t("battle_ext.tab_history")}
          </button>
        </div>
          ...
          <div className="space-y-6">
            {!user && !isLoading && (
              <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-8 text-center">
                ...
                <p className="mb-3 text-sm text-white/50">{t("battle_ext.login_prompt").replace('%{mode}', tab === "ranked" ? "Ranked" : "Casual")}</p>
                <Link href="/login" className="inline-flex items-center gap-2 rounded-lg bg-lyoko-blue/20 px-5 py-2 text-sm font-semibold text-lyoko-blue hover:bg-lyoko-blue/30">
                  <LogIn size={14} /> {t("battle_ext.btn_login")}
                </Link>
              </div>
            )}
              ...
              <>
                ...
                <div className="rounded-2xl border p-6" style={{ borderColor: `${MODE_META[tab].color}30`, background: `${MODE_META[tab].color}05` }}>
                  <div className="mb-4 flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="font-display text-lg font-bold text-white">{t(`battle_ext.modes.${tab}`)}</h2>
                      <p className="text-xs text-white/50">{t(`battle_ext.mode_descs.${tab}`)}</p>
                    </div>
                  </div>

                  {/* Difficulty selector */}
                  <p className="mb-2 text-xs font-medium text-white/40">{t("battle_ext.difficulty_label")}</p>
                    ...
                    {(["easy", "medium", "hard"] as const).map(d => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center gap-2 rounded-lg border px-4 py-2.5 text-sm font-medium transition-all ${difficulty === d ? "border-current" : "border-white/10 text-white/50 hover:border-white/20"
                            }`}
                          style={difficulty === d ? { color: m.color, borderColor: m.color, background: `${m.color}10` } : {}}
                        >
                          <Icon size={14} /> {t(`battle_ext.difficulties.${d}`)}
                  ...
                  {!socket.connected && (
                    <div className="mb-3 rounded-lg border border-xana-red/30 bg-xana-red/10 px-4 py-3 text-sm text-xana-red">
                      <span className="font-semibold">
                        {socket.connectionError ? t("battle_ext.error_socket").replace('%{err}', socket.connectionError) : t("battle_ext.connecting")}
                      </span>
                      {!socket.connectionError && t("battle_ext.wait_queue")}
                    </div>
                  )}
                  {socket.inQueue ? (
                    <div className="space-y-3">
                      <div className="flex items-center gap-4 rounded-xl border border-white/10 bg-dark-bg/50 p-4">
                        ...
                        <div className="flex-1">
                          <p className="text-sm font-semibold text-white">{t("battle_ext.searching")}</p>
                          <p className="text-xs text-white/40">
                            {tab === "ranked" ? "Ranked" : "Casual"} · {t(`battle_ext.difficulties.${difficulty}`)} · {searchTime}s
                          </p>
                        </div>
                        <button
                      ...
                      {socket.queueInfo && (
                        <p className="text-center text-xs text-white/30">
                          {(() => {
                            ...
                            return t("battle_ext.players_in_queue").replace('%{count}', (q as any)[difficulty].toString());
                          })()}
                        </p>
                      )}
                    </div>
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 rounded-lg px-8 py-3 text-sm font-bold text-dark-bg transition-all hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0"
                      style={{ background: `linear-gradient(135deg, ${MODE_META[tab].color}, ${MODE_META[tab].color}cc)` }}
                    >
                      <Search size={16} /> {socket.connected ? t("battle_ext.btn_find") : t("battle_ext.btn_connecting")}
                    </button>
                  )}
                </div>
                    ...
                    {(["easy", "medium", "hard"] as const).map(d => {
                      ...
                      return (
                        <div key={d} className="rounded-xl border border-white/8 bg-white/[0.02] p-3 text-center">
                          ...
                          <p className="text-[0.65rem] text-white/40">{t("battle_ext.in_queue").replace('%{diff}', t(`battle_ext.difficulties.${d}`))}</p>
                        </div>
                      );
                    })}
                  </div>
        ...
        {tab === "friends" && (
          <div className="space-y-6">
            {!user && !isLoading && (
              <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-8 text-center">
                <LogIn className="mx-auto mb-3 h-10 w-10 text-white/20" />
                <p className="mb-3 text-sm text-white/50">{t("battle_ext.login_prompt_friends")}</p>
              ...
              <div className="grid gap-6 md:grid-cols-2">
                ...
                <div className="rounded-2xl border border-carthage-gold/20 bg-carthage-gold/5 p-6">
                  <h3 className="mb-3 flex items-center gap-2 font-display text-lg font-bold text-white">
                    <Zap size={18} className="text-carthage-gold" /> {t("battle_ext.create_battle")}
                  </h3>

                  <p className="mb-3 text-xs font-medium text-white/40">{t("battle_ext.difficulty_label")}</p>
                    ...
                    {(["easy", "medium", "hard"] as const).map(d => {
                      ...
                      return (
                        <button
                          key={d}
                          onClick={() => setDifficulty(d)}
                          className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition-all ${difficulty === d ? "border-current" : "border-white/10 text-white/50"
                            }`}
                          style={difficulty === d ? { color: m.color, borderColor: m.color, background: `${m.color}10` } : {}}
                        >
                          <Icon size={12} /> {t(`battle_ext.difficulties.${d}`)}
                        </button>
                      );
                    })}
                  </div>
                  ...
                  {friendBattleId ? (
                    <div className="space-y-3">
                      <p className="text-xs text-white/50">{t("battle_ext.share_code")}</p>
                      ...
                      >
                        <ArrowRight size={14} /> {t("battle_ext.btn_arena")}
                      </Link>
                    </div>
                  ) : (
                    <button
                ...
                <div className="rounded-2xl border border-lyoko-blue/20 bg-lyoko-blue/5 p-6">
                  <h3 className="mb-3 flex items-center gap-2 font-display text-lg font-bold text-white">
                    <UserPlus size={18} className="text-lyoko-blue" /> {t("battle_ext.join_battle")}
                  </h3>
                  <p className="mb-3 text-xs text-white/50">{t("battle_ext.enter_code")}</p>
                  <div className="mb-4 flex gap-2">
                    <input
                      type="text"
                      value={friendCode}
                      onChange={e => setFriendCode(e.target.value)}
                      placeholder={t("battle_ext.code_placeholder")}
                    ...
                    >
                      <ArrowRight size={14} /> {t("battle_ext.btn_join")}
                    </button>
                  </div>
                </div>
              </div>
        ...
        {tab === "leaderboard" && (
          <div>
            <h2 className="mb-4 flex items-center gap-2 font-display text-lg font-bold text-white">
              <Award size={20} className="text-carthage-gold" /> {t("battle_ext.leaderboard_title")}
            </h2>
            {leaderboard.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-white/10 py-16 text-center">
                <Trophy className="mx-auto mb-3 h-12 w-12 text-white/10" />
                <p className="text-sm text-white/40">{t("battle_ext.leaderboard_empty")}</p>
              </div>
            ) : (
              <div className="space-y-1.5">
                {leaderboard.map((p: any, i: number) => {
                  ...
                  return (
                    <div
                      key={p.id}
                      className={`flex items-center gap-4 rounded-xl border p-3.5 transition-all ${isMe ? "border-lyoko-purple/30 bg-lyoko-purple/5" : "border-white/5 bg-white/[0.02] hover:border-white/10"
                        }`}
                    >
                      <div className={`flex h-8 w-8 items-center justify-center rounded-full text-sm font-bold ${i === 0 ? "bg-carthage-gold/20 text-carthage-gold" : i === 1 ? "bg-white/10 text-white/70" : i === 2 ? "bg-orange-500/15 text-orange-400" : "text-white/30"
                        }`}>
                        {i < 3 ? ["🥇", "🥈", "🥉"][i] : `#${i + 1}`}
                      </div>
                      <div
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-semibold text-white">{p.display_name} {isMe && <span className="text-xs text-lyoko-purple">({t("battle_ext.you")})</span>}</p>
                        <p className="text-xs text-white/40">@{p.username} · {t("battle_ext.lvl")}{p.level}</p>
                      </div>
        ...
        {tab === "history" && (
          <div>
            <h2 className="mb-4 flex items-center gap-2 font-display text-lg font-bold text-white">
              <BarChart3 size={20} className="text-lyoko-blue" /> {t("battle_ext.history_title")}
            </h2>
            {!user ? (
              <p className="py-8 text-center text-sm text-white/40">{t("battle_ext.history_login")}</p>
            ) : history.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-white/10 py-16 text-center">
                <Trophy className="mx-auto mb-3 h-12 w-12 text-white/10" />
                <p className="text-sm text-white/40">{t("battle_ext.history_empty")}</p>
              </div>
            ) : (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-medium text-white">
                          {h.challenge_title}
                          <span className="ml-2 text-xs font-normal" style={{ color: diff.color }}>{t(`battle_ext.difficulties.${h.challenge_difficulty || 'easy'}`)}</span>
                      ...
                      <span className={`rounded-md px-2.5 py-1 text-xs font-bold ${won ? "bg-lyoko-green/15 text-lyoko-green" : "bg-xana-red/15 text-xana-red"}`}>
                        {won ? t("battle_ext.win") : t("battle_ext.loss")}
                      </span>
