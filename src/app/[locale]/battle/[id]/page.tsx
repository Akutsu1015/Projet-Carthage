  ...
  if (!battle || !challenge) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4">
        ...
        <p className="text-white/40">{t("battle_ext.arena_not_found")}</p>
        <Link href="/battle" className="text-sm text-xana-red hover:underline">← {t("battle_ext.back_lobby")}</Link>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-[calc(100vh-4rem)]">
        <div className="mx-auto max-w-2xl px-4 py-16 text-center lg:px-8">
          ...
          <h1 className="mb-2 font-display text-2xl font-extrabold text-white">{t("battle_ext.waiting_opponent")}</h1>
          <p className="mb-1 text-sm text-white/50">
            {battle.mode === "friends" ? t("battle_ext.waiting_friends") : t("battle_ext.waiting_matchmaking")}
          </p>
          ...
          <div className="mb-6 mt-6 rounded-2xl border border-white/8 bg-white/[0.02] p-5 text-left">
            <div className="mb-2 flex items-center gap-2">
              <span className="rounded-md px-2 py-0.5 text-xs font-semibold" style={{ color: diff.color, background: `${diff.color}15` }}>{t(`battle_ext.difficulties.${challenge.difficulty || 'easy'}`)}</span>
          ...
          {battle.mode === "friends" && (
            <div className="mb-6">
              <p className="mb-2 text-xs text-white/40">{t("battle_ext.invite_code")}</p>
              <div className="flex items-center justify-center gap-2">
                ...
                <button onClick={handleCopyLink} className="flex items-center gap-1.5 rounded-lg bg-lyoko-blue/15 px-3 py-2 text-xs font-semibold text-lyoko-blue hover:bg-lyoko-blue/25">
                  {copied ? <><Check size={13} /> {t("battle_ext.copied")}</> : <><Copy size={13} /> {t("battle_ext.copy_link")}</>}
                </button>
              </div>
            </div>
          )}
          ...
          <div className="mb-4 flex items-center justify-center gap-2">
            ...
            <span className="text-sm text-white/40">{t("battle_ext.searching")}</span>
          </div>

          {isCreator && (
            <button onClick={handleCancel} className="text-sm text-white/30 hover:text-xana-red">
              {t("battle_ext.cancel_battle")}
            </button>
          )}
          ...
          {!isParticipant && user && (
            ...
            >
              <Swords size={16} /> {t("battle_ext.join_this_battle")}
            </button>
          )}
        </div>
      </div>
      ...
      <div className="min-h-[calc(100vh-4rem)]">
        <div className="mx-auto max-w-3xl px-4 py-12 lg:px-8">
          <Link href="/battle" className="mb-6 inline-flex items-center gap-1.5 text-sm text-white/40 hover:text-white/70">
            <ArrowLeft size={14} /> {t("battle_ext.back_lobby")}
          </Link>
          ...
          <div className="mb-8 text-center">
            ...
            <h1 className="mb-2 font-display text-3xl font-extrabold">
              {iWon ? (
                <span className="text-lyoko-green">{t("battle_ext.victory")}</span>
              ) : isParticipant ? (
                <span className="text-xana-red">{t("battle_ext.defeat")}</span>
              ) : (
                <span className="text-white">{t("battle_ext.battle_ended")}</span>
              )}
            </h1>
            <p className="text-sm text-white/50">
              {challenge.title} · {t(`battle_ext.difficulties.${challenge.difficulty || 'easy'}`)}
          ...
          <div className="mb-6 grid grid-cols-2 gap-4">
            {[
              ...
              <div key={i} className={`rounded-2xl border p-5 text-center ${isWinner ? "border-lyoko-green/30 bg-lyoko-green/5" : "border-white/8 bg-white/[0.02]"}`}>
                ...
                {isMe && <span className="mb-1 inline-block rounded-full bg-lyoko-blue/15 px-2 py-0.5 text-[0.6rem] font-semibold text-lyoko-blue">{t("battle_ext.you")}</span>}
                <div
                ...
                <p className="text-sm font-semibold text-white">{player?.displayName || t("battle_ext.opponent")}</p>
                <p className="text-xs text-white/40">{t("battle_ext.lvl")} {player?.level || "?"}</p>
                <p className="mt-2 font-display text-2xl font-extrabold" style={{ color: isWinner ? "#00ff88" : "#ff2244" }}>
                  {!isWinner && !time && battle.creator_time !== null && battle.opponent_time !== null ? t("battle_ext.forfeit") : time ? `${(time / 1000).toFixed(1)}s` : "—"}
                </p>
              </div>
            ))}
          </div>
          ...
          <div className="flex justify-center gap-3">
            ...
            >
              {copied ? <><Check size={14} /> {t("battle_ext.copied")}</> : <><Share2 size={14} /> {t("battle_ext.share")}</>}
            </button>
          </div>
        </div>
      </div>
    ...
    <div className="flex h-[calc(100vh-4rem)] flex-col">
      ...
      <div className="flex items-center gap-3 border-b border-white/5 bg-dark-surface px-4 py-2">
        ...
        {antiCheat.flags.flagged && (
          <div className="flex items-center gap-1 rounded-lg bg-xana-red/10 px-2 py-1 text-xs text-xana-red" title={`Flags: ${antiCheat.flags.flags.join(", ")}`}>
            ...
            <span>{t("battle_ext.cheat_detected")}</span>
          </div>
        )}
        {!antiCheat.flags.flagged && antiCheat.flags.tabSwitches > 0 && (
          <div className="flex items-center gap-1 rounded-lg bg-carthage-gold/10 px-2 py-1 text-xs text-carthage-gold" title={`${antiCheat.flags.tabSwitches} ${t("battle_ext.tab_switches")}`}>
            <Eye size={12} />
        ...
        {!submitted && (
          ...
          >
            <Flag size={11} /> {t("battle_ext.abandon")}
          </button>
        )}
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-[380px] flex-shrink-0 overflow-y-auto border-r border-white/5 bg-dark-bg p-5">
          ...
          <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-white/30">{t("battle_ext.tests")}</h3>
          <div className="space-y-2">
            {JSON.parse(challenge.tests || "[]").map((t: any, i: number) => {
              const result = testResults[i];
              return (
                <div key={i} className={`rounded-lg border p-3 text-xs font-mono ${result ? (result.pass ? "border-lyoko-green/30 bg-lyoko-green/5" : "border-xana-red/30 bg-xana-red/5") : "border-white/8 bg-white/[0.02]"
                  }`}>
                  <div className="flex items-center gap-2">
                    {result && (result.pass ? <Check size={12} className="text-lyoko-green" /> : <X size={12} className="text-xana-red" />)}
                    <span className="text-white/50">{t("battle_ext.input")}</span>
                    <span className="text-white/80">{t.input}</span>
                  </div>
                  <div className="mt-1 flex items-center gap-2">
                    <span className="text-white/50">{t("battle_ext.expected")}</span>
                    <span className="text-lyoko-green">{t.expected}</span>
                  </div>
                  {result && !result.pass && (
                    <div className="mt-1 flex items-center gap-2">
                      <span className="text-white/50">{t("battle_ext.got")}</span>
          ...
          {opponentDone && !submitted && (
            <div className="mt-4 rounded-xl border border-carthage-gold/30 bg-carthage-gold/10 p-3 text-center">
              <p className="text-xs font-semibold text-carthage-gold">⚡ {t("battle_ext.opponent_done")}</p>
              <p className="text-[0.65rem] text-white/40">
                {socket.opponentSubmitted ? t("battle_ext.in_time").replace('%{time}', (socket.opponentSubmitted.timeMs / 1000).toFixed(1)) : "—"}
              </p>
            </div>
          )}
          ...
          {submitted && (
            <div className="mt-4 rounded-xl border border-lyoko-green/30 bg-lyoko-green/10 p-4 text-center">
              ...
              <p className="text-sm font-semibold text-lyoko-green">{t("battle_ext.solution_submitted")}</p>
              <p className="text-xs text-white/40">{t("battle_ext.waiting_opponent_result")}</p>
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-3 border-t border-white/5 bg-dark-surface px-4 py-2.5">
            ...
            >
              <Play size={14} /> {t("battle_ext.test_btn")}
            </button>
            <div className="flex-1" />
            {testResults.length > 0 && (
              <span className={`text-xs font-medium ${testResults.every(r => r.pass) ? "text-lyoko-green" : "text-xana-red"}`}>
                {t("battle_ext.tests_passed").replace('%{pass}', testResults.filter(r => r.pass).length.toString()).replace('%{total}', testResults.length.toString())}
              </span>
            )}
            <button
            ...
            >
              ...
              {submitting ? t("battle_ext.btn_submitting") : t("battle_ext.btn_submit")}
            </button>
          </div>
        </div>
      </div>
