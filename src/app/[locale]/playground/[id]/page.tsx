  ...
  if (!creation) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4">
        ...
        <p className="text-white/40">{t("playground_ext.not_found")}</p>
        <Link href="/playground" className="text-sm text-lyoko-purple hover:underline">← {t("playground_ext.back_gallery")}</Link>
      </div>
    );
  }

  const TABS: { key: EditorTab; label: string; icon: any; color: string }[] = [
      ...
      <div className="border-b border-white/5 bg-dark-surface px-4 py-4 lg:px-8">
        <div className="mx-auto max-w-6xl">
          <Link href="/playground" className="mb-3 inline-flex items-center gap-1.5 text-sm text-white/40 hover:text-white/70">
            <ArrowLeft size={14} /> {t("playground_ext.back_gallery")}
          </Link>
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-white/30">@{creation.author?.username} · {t("playground_ext.lvl")}{creation.author?.level}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={handleLike}
                className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-sm font-medium transition-all ${creation.liked
              ...
              >
                {copied ? <><Check size={13} /> {t("playground_ext.copied")}</> : <><Share2 size={13} /> {t("playground_ext.share")}</>}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-4" style={{ height: "calc(100vh - 280px)", minHeight: 400 }}>
          ...
          <div className="flex w-1/2 flex-col overflow-hidden rounded-xl border border-white/10">
            <div className="flex items-center gap-2 border-b border-white/5 bg-dark-surface px-3 py-1.5">
              ...
              <span className="text-[0.7rem] font-medium text-white/50">{t("playground_ext.preview")}</span>
            </div>
            ...
            <div className="flex border-b border-white/5 bg-dark-surface">
              {TABS.map(({ key, label, icon: Icon, color }) => (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-4 py-2 text-xs font-medium transition-all ${activeTab === key ? "border-b-2 text-white" : "text-white/40 hover:text-white/60"
                    }`}
