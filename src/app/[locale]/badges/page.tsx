      ...
      <div className="border-b border-white/5 bg-gradient-to-br from-dark-surface to-dark-bg py-8">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-display text-2xl font-bold text-white">{t("badges.title")}</h1>
              <p className="text-sm text-white/50">{unlockedCount}/{BADGES.length} {t("badges.unlocked")}</p>
            </div>
          </div>
        </div>
      </div>
          ...
          return (
            <section key={cat}>
              <div className="mb-4 flex items-center gap-2">
                <h2 className="font-display text-lg font-bold text-white">{t(`badges.categories.${cat}`)}</h2>
              ...
              <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
                {catBadges.map((badge) => {
                  ...
                  return (
                    <div
                      key={badge.id}
                      className={`relative overflow-hidden rounded-2xl border p-5 text-center transition-all ${unlocked
                          ? "border-white/15 bg-white/[0.05] shadow-lg"
                          : "border-white/5 bg-white/[0.02] opacity-60"
                        }`}
                    >
                      <div
                        className={`mx-auto mb-3 flex h-16 w-16 items-center justify-center rounded-2xl border ${unlocked ? "border-opacity-30" : "border-white/10 bg-white/5"
                          }`}
