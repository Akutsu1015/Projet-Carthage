...
function Podium({ members, userId }: { members: LeagueMember[]; userId: number }) {
  ...
  const podiumLabels = t("leaderboard_ext.podium") as unknown as string[];
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className={`truncate text-sm font-medium ${isUser ? "text-[#00d4ff]" : "text-white/80"}`}>
          {member.displayName}
          {isUser && <span className="ml-1.5 text-[0.6rem] text-[#00d4ff]/60">({t("leaderboard_ext.you")})</span>}
        </p>
        <p className="text-[0.65rem] text-white/30">{t("leaderboard_ext.lvl")} {member.level}</p>
      </div>
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-[0.65rem] text-white/30">
          {t("leaderboard_ext.lvl")} {entry.level} &middot; {entry.exerciseCount} {t("leaderboard_ext.exercises")}
        </p>
      </div>
              ...
              <div className="p-5">
                ...
                <div className="flex items-center justify-center gap-2">
                  ...
                  <h2 className="font-display text-xl font-bold" style={{ color: meta.color }}>
                    {t("leaderboard_ext.league_prefix")} {t(`leaderboard_ext.leagues.${leagueData.league}`)}
                  </h2>
                </div>
                ...
                <div className="mt-4 flex justify-center gap-6 text-center">
                  <div>
                    ...
                    <p className="text-[0.65rem] text-white/40">{t("leaderboard_ext.days_left")}</p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-[0.65rem] text-white/40">{t("leaderboard_ext.participants")}</p>
                  </div>
                  <div className="h-8 w-px bg-white/10" />
                  <div>
                    <div className="flex items-center justify-center gap-1">
                    ...
                    <p className="text-[0.65rem] text-white/40">{t("leaderboard_ext.your_rank")}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-4 border-t border-white/[0.04] px-5 py-2.5">
                {leagueData.promotionZone > 0 && (
                  <div className="flex items-center gap-1.5 text-[0.65rem] text-green-400/70">
                    <ChevronUp size={12} />
                    <span>{t("leaderboard_ext.top_promoted").replace('%{n}', leagueData.promotionZone.toString())}</span>
                  </div>
                )}
                {leagueData.demotionZone > 0 && (
                  <div className="flex items-center gap-1.5 text-[0.65rem] text-red-400/70">
                    <ChevronDown size={12} />
                    <span>{t("leaderboard_ext.last_demoted").replace('%{n}', leagueData.demotionZone.toString())}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              {leagueData.members.length === 0 && (
                <div className="py-16 text-center">
                  ...
                  <p className="text-sm text-white/40">{t("leaderboard_ext.empty_league")}</p>
                  <p className="text-xs text-white/25">{t("leaderboard_ext.empty_league_sub")}</p>
                </div>
              )}
            </div>
          </>
          ...
          <>
            ...
            {globalUserRank && (
              <div className="mb-6 rounded-2xl border border-white/[0.06] bg-white/[0.02] p-4 text-center">
                <p className="text-xs text-white/40">{t("leaderboard_ext.your_global_rank")}</p>
            ...
            <div className="space-y-1">
              ...
              {globalData.length === 0 && (
                <div className="py-16 text-center">
                  ...
                  <p className="text-sm text-white/40">{t("leaderboard_ext.empty_global")}</p>
                  <p className="text-xs text-white/25">{t("leaderboard_ext.empty_global_sub")}</p>
                </div>
              )}
            </div>
          </>
