...
const BATTLE_SERVER_URL = typeof window !== "undefined"
  ? (window.location.hostname === "localhost"
    ? "http://localhost:3002"
    : window.location.origin)  // Use same origin for nginx reverse proxy
  : "";
...
export function useBattleSocket(sessionToken?: string | null) {
  ...
  useEffect(() => {
    ...
    const fetchToken = async () => {
      try {
        console.log("[BattleSocket] Fetching battle token...");
        const res = await fetch("/api/db/battle-token", { credentials: "include" });
        if (!res.ok) {
          console.error("[BattleSocket] Failed to fetch token:", res.status);
          return;
        }
        const data = await res.json();
        if (data.success && data.token) {
          console.log("[BattleSocket] Token received");
          setToken(data.token);
        } else {
          console.warn("[BattleSocket] No token in response:", data);
        }
      } catch (err) {
        console.error("[BattleSocket] Error fetching token:", err);
      }
    };
    
    fetchToken();
    // Retry every 5 seconds if no token
  ...
  useEffect(() => {
    ...
    socket.on("error", (err) => {
      console.error("[BattleSocket] Socket error:", err);
    });
  ...
  const joinQueue = useCallback((mode: string, difficulty: string) => {
    ...
    console.log("[BattleSocket] Joined queue:", mode, difficulty);
  }, []);
