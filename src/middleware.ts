...
const RL: Record<string, RLConfig> = {
  auth: { windowMs: 60_000, max: 200 }, // Augmenté pour éviter les blocages lors de l'OAuth
  execute: { windowMs: 60_000, max: 100 },
  battle_write: { windowMs: 60_000, max: 50 },
  api_write: { windowMs: 60_000, max: 100 },
  api_read: { windowMs: 60_000, max: 500 },
...
function rateLimit(ip: string, bucket: string, cfg: RLConfig): { ok: boolean; remaining: number; retryAfter: number } {
  ...
  if (ts.length >= cfg.max) {
    ...
    console.warn(`[RateLimit] Blocked ${key} (bucket: ${bucket}, retry: ${retryAfter}s)`);
...
function markSuspicious(ip: string) {
  ...
  e.count++;
  if (e.count > 50) {
    console.error(`[Ban] IP banned for 1h: ${ip}`);
...
const GOOD_BOTS = [
  ...
  /dart/i, /flutter/i, /projetcarthage/i,
];
...
function getBucket(pathname: string, method: string): { bucket: string; cfg: RLConfig } {
  ...
  if (pathname === "/api/db/execute" || pathname === "/api/db/validate")
